...
def call(body) {
    ...
    if (!s2iMode){
        if (flow.isSingleNode()){
            ...
            def command = "docker tag ${user}/${artifactId}:${config.version} ${env.FABRIC8_DOCKER_REGISTRY_SERVICE_HOST}:${env.FABRIC8_DOCKER_REGISTRY_SERVICE_PORT}/${user}/${artifactId}:${config.version}"

            def out = sh script: command,returnStdOut:true
            echo "result for docker tag = ${out}"
